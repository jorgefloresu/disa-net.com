...
function(e){e.onclick=function(){save_focus(e);if(e.getAttribute('aspect')=='process')
JsHttpRequest.request(this,null,30000);else
...
'a.printlink':function(l){l.onclick=function(){save_focus(this);JsHttpRequest.request(this);return false;}
},
...
else if(_hotkeys.alt&&((key>47&&key<58)||(key>64&&key<91))){var n=_hotkeys.focus;var l=document.links;var cnt=l.length;key=String.fromCharCode(key);for(var i=0;i<cnt;i++){n=(n+1)%cnt;if(l[n].accessKey==key&&l[n].scrollWidth){_hotkeys.focus=n;var tmp=function(){document.links[_hotkeys.focus].focus();};setTimeout(tmp,0);break;}
}
return stopEv(ev);}
if((ev.ctrlKey&&key==13)||key==27){_hotkeys.alt=false;_hotkeys.focus=-1;ev.cancelBubble=true;if(ev.stopPropagation)ev.stopPropagation();for(var j=0;j<this.forms.length;j++){var form=this.forms[j];for(var i=0;i<form.elements.length;i++){var el=form.elements[i];var asp=el.getAttribute('aspect');if(el.className!='editbutton'&&asp=='selector'&&(key==13||key==27)){passBack(key==13 ? el.getAttribute('rel'):false);ev.returnValue=false;return false;}
if((asp=='default'&&key==13)||(asp=='cancel'&&key==27)){JsHttpRequest.request(el);ev.returnValue=false;return false;}
}
}
