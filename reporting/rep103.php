...
function print_customer_details_listing()
{
	...
	while ($myrow=db_fetch($result)) 
	{
		...
		if ($printcustomer)
		{
			...
			$rep->NewLine();
                        $debtor_name_ref = $myrow['debtor_ref']. " - " . $myrow['name'];
			$rep->TextCol(0, 1, $debtor_name_ref);
			$adr = Explode("\n", $myrow['address']);
			$count1 = count($adr);
			for ($i = 0; $i < $count1; $i++)
				$rep->TextCol(0, 1, $adr[$i], 0, ($i + 1) * $rep->lineHeight);
                        // JAFU - dividir la dirección del cliente en dos partes
                        //$long = intval(strlen($myrow['address']) / 2);
                        //$adr1 = substr($myrow['address'], 0, $long * -1);
                        //$adr2 = substr($myrow['address'], $long);
                        //$rep->TextCol(0, 1, $adr1, 0, $rep->lineHeight);
                        //$rep->TextCol(0, 1, $adr2, 0, 2* $rep->lineHeight);
