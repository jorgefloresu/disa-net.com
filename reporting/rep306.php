...
function getTransactions($category, $location, $fromcust, $from, $to)
{
	...
	$sql = "SELECT ".TB_PREF."stock_master.category_id,
			...
			SUM(-".TB_PREF."stock_moves.qty/".TB_PREF."purch_data.conversion_factor) AS qty,
		...
		AND ".TB_PREF."stock_moves.trans_no=".TB_PREF."debtor_trans.trans_no
                AND ".TB_PREF."stock_master.stock_id=".TB_PREF."purch_data.stock_id 
		...
		if ($fromcust != -1)
			$sql .= " AND ".TB_PREF."debtors_master.debtor_no = ".db_escape($fromcust);
                ...
                // JAFU : Descartar el agrupamiento por cliente
		...
		$sql .= " GROUP BY ".TB_PREF."stock_master.category_id, ".TB_PREF."stock_master.stock_id ORDER BY ".TB_PREF."stock_master.category_id,
			".TB_PREF."stock_master.stock_id";
...
function print_inventory_sales()
{
	...
	// JAFU : Eliminando columna de cliente. 
        // orig: $cols = array(0, 75, 175, 250, 300, 375, 450,	515);
	$cols = array(0, 75, 250, 300, 375, 450, 515);

	// orig: $headers = array(_('Category'), _('Description'), _('Customer'), _('Qty'), _('Sales'), _('Cost'), _('Contribution'));
	$headers = array(_('Category'), _('Description'), _('Qty'), _('Sales'), _('Cost'), _('Contribution'));
	//if ($fromcust != ALL_NUMERIC)
	//	$headers[2] = '';	

	// orig: $aligns = array('left', 'left', 'left', 'right', 'right', 'right', 'right');
    ...
    // JAFU : Cambio de titulo. 
    // Orig:$rep = new FrontReport(_('Inventory Sales Report'), "InventorySalesReport", user_pagesize());
    $rep = new FrontReport(_('Reporte de Movimientos de Inventario'), "InventorySalesReport", user_pagesize());
	...
	while ($trans=db_fetch($res))
	{
		...
		// JAFU : Mostrar siempre la descripcion en el mismo ancho
                //if ($fromcust == ALL_NUMERIC)
		//{
			$rep->TextCol(1, 2, $trans['description']);
			// JAFU : no mostrar cliente. $rep->TextCol(2, 3, $trans['debtor_name']);
		//}
		//else
		//	$rep->TextCol(1, 3, $trans['description']);
		$rep->AmountCol(2, 3, $trans['qty'], get_qty_dec($trans['stock_id']));
		$rep->AmountCol(3, 4, $trans['amt'], $dec);
		$rep->AmountCol(4, 5, $trans['cost'], $dec);
		$rep->AmountCol(5, 6, $cb, $dec);
		$rep->fontsize += 2;
		$total += $trans['amt'];
		$total1 += $trans['cost'];
		$total2 += $cb;
